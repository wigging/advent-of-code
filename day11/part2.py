"""
Part 2 of Day 11 for Advent of Code 2023.
...
def run_part2(input_file, display_grid=False):
    ...
    for i, (gal_row, gal_col) in enumerate(galaxies):
        for gal_row2, gal_col2 in galaxies[:i]:
            for row in range(min(gal_row2, gal_row), max(gal_row2, gal_row)):
                total_length += 1_000_000 if row in empty_rows else 1

            for col in range(min(gal_col2, gal_col), max(gal_col2, gal_col)):
                total_length += 1_000_000 if col in empty_cols else 1
...
def main():
    ...
    # run_part2("input_ex.txt", display_grid=True)
